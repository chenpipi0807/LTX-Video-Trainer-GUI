...
if __name__ == "__main__":
    # 命令行参数处理
    import argparse
    parser = argparse.ArgumentParser(description="LTX-Video-Trainer最小化UI")
    parser.add_argument("--port", type=int, default=7860, help="服务器端口号")
    parser.add_argument("--host", type=str, default="0.0.0.0", help="服务器主机地址")
    parser.add_argument("--share", action="store_true", help="是否启用Gradio分享功能")
    args = parser.parse_args()
    
    # 创建并启动UI
    app = create_ui()
    logger.info(f"在端口 {args.port} 上启动服务器")
    app.launch(server_name=args.host, server_port=args.port, share=args.share, inbrowser=False)
