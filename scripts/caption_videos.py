...
The paths to videos in the generated dataset/captions file will be FILE NAMES ONLY,
not relative paths. This makes the dataset more compatible with preprocessing scripts.
...
def caption_media(
    ...
    for rel_path, caption in existing_captions.items():
        ...
        try:
            ...
            # 如果是纯文件名，没有路径信息，则直接使用
            existing_captions_abs[rel_path] = caption
    ...
    for media_file in media_files:
        ...
        if not override and (media_path_str in existing_captions_abs or file_name in existing_captions):
    ...
    with progress:
        ...
        for media_file in media_to_process:
            ...
            try:
                ...
                console.print(f"[green]✓[/] Using filename as key: {file_name}")
...
def _load_existing_captions(  # noqa: PLR0912
    ...
    try:
        match format_type:
            case OutputFormat.TXT:
                # For TXT format, we need to find the corresponding paths file
                captions_file = output_path.with_stem(f"{output_path.stem}_captions")
                paths_file = output_path.with_stem(f"{output_path.stem}_paths")
                ...
                if captions_file.exists() and paths_file.exists():
                    ...
                    if len(captions) == len(paths):
                        for media_path, caption in zip(paths, captions):
                            existing_captions[media_path] = caption
                    else:
                        console.print("[bold yellow]Warning: Mismatch between captions and paths files.[/]")
            ...
            case OutputFormat.CSV:
                with output_path.open("r", encoding="utf-8", newline="") as f:
                    ...
                    # Find indices of caption and media_path columns
                    caption_index = header.index("caption") if "caption" in header else 0
                    media_path_index = header.index("media_path") if "media_path" in header else 1

                    for row in reader:
                        if len(row) > max(caption_index, media_path_index):
                            existing_captions[row[media_path_index]] = row[caption_index]
